  ...
  const data = [
    {
      ...
      description: `Built with Next.js 15 (App Router), TypeScript, Tailwind CSS v4, and Supabase for authentication, database, and file storage, this platform includes a public website with a live gallery carousel, artist roster, portfolio viewer, booking form, aftercare guide, and a contact form that sends emails via Gmail/Nodemailer. It features a booking system where customers submit appointment requests that the studio can review and manage, along with Supabase Auth for secure logins so customers can view their booking history. A protected admin CMS dashboard allows staff to manage artists with image uploads to Supabase Storage, update galleries, manage studio photos, and handle bookings without coding. The system also integrates Google Generative AI (@google/genai) and includes server and client Supabase clients, middleware-based route protection, cloud image storage with public URLs, real-time data from Supabase Postgres, and a fully responsive dark luxury UI with custom animations and a Cinzel serif brand identity.`,
